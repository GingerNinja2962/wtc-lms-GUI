...
class reviewsMenuClass(baseWindowClass):
    ...
    def __init__(self):
        ...
        self.layout = reviews.layout.reviewsLayout()
    ...
    def read(self):
        ...
        self.oldResults = self.checkChanges()
        ...
        if core.tokenCheck("review") or self.values["-SETTINGS-MENU-"] == "Update Reviews":
            if self.values["-SETTINGS-MENU-"] == "Update Reviews":
            ...
            reviews.layout.updateCounterFrame(self.window)
    ...
    def checkChanges(self):
        ...
        """
        results = [
        self.values["-PROBLEM-1-"],
        self.values["-PROBLEM-2-"],
        ...
        if self.results == self.oldResults[1::]:
            self.oldResults = [False] + results

        elif (self.values["-PROBLEM-1-"] == [] and self.values["-PROBLEM-2-"] == []\
                    and self.values["-TOGGLE-ALL-"] == False):
            self.window['-OUTPUT-'].update('')
            self.oldResults = [False] + results

        else:
            if results[2::] == self.oldResults[3::]:
                if self.values["-PROBLEM-1-"] == self.oldResults[1] and self.values["-PROBLEM-2-"] != []:
                    self.window['-PROBLEM-1-'].set_value([])
                elif self.values["-PROBLEM-2-"] == self.oldResults[2] and self.values["-PROBLEM-1-"] != []:
                    self.window['-PROBLEM-2-'].set_value([])
